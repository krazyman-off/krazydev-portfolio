...
if(c){
  ...
  (function loop(){
    ...
    pts.forEach(p=>{
      ...
      ctx.fillStyle="rgba(168,85,247,.55)"; ctx.fill();
    });
    for(let i=0;i<N;i++) for(let j=i+1;j<N;j++){
      const a=pts[i], b=pts[j], d=Math.hypot(a.x-b.x,a.y-b.y);
      if(d<150){ ctx.strokeStyle=`rgba(168,85,247,${(1-d/150)*.16})`; ctx.beginPath(); ctx.moveTo(a.x,a.y); ctx.lineTo(b.x,b.y); ctx.stroke(); }
    }
